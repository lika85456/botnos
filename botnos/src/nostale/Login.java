...
public class Login{
	...
	public Player login(String nickname,String password,Server country,int server,int channel,int character) {
		...
		try {
			...
			// Make login packet
			String login_packet = "";
			login_packet = "NoS0575 3001742 " + nickname + " " + Crypto.encrypt(password).toUpperCase()
					+ " 0080B9C9";
			login_packet += (char) (11);
			login_packet += version + " 0 " + (Crypto.md5(HASH + nickname).toUpperCase());
			...
			for (GameServer gs : channels) {
				if (gs.channel.equals(String.valueOf(channel))
						&& gs.server.equals(String.valueOf(server))) {
					selectServer(gs);
					selectChar(characters.get(character));
	...
	private void selectChar(int slot) {
	...
	private void selectServer(GameServer s) {
		...
		try {
			...
			Thread.sleep(200);
			packetId++;
			c.send(Crypto.EncryptGamePacket(packetId +" "+ nickname, this.session, false)
					+ Crypto.EncryptGamePacket((packetId + 1 )+" "+ password, this.session, false));
			packetId+=2;
			parseChars();
