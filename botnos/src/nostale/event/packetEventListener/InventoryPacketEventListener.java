	...
	public void packetCall(Packet p) {
		...
		case "inv":
			...
			for (InventoryItemInstance tI : invPacket.items) {
				if (inventoryHandler.inventory.get(tI.InventoryType) == null)
					inventoryHandler.inventory.put(tI.InventoryType, new HashMap<Short, InventoryItemInstance>());
		...
		case "ivn":
			...
			// if(inventory.get(tI.InventoryType)==null)
			// inventory.put(tI.InventoryType, new
			// HashMap<Short,InventoryItemInstance>());
