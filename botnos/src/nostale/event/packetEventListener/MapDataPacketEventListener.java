...
import nostale.event.gameEvent.AdminOnMapGameEvent;
	...
	public void packetCall(Packet packet) {
		...
		case "in":
			...
			if (inPacket.userType == UserType.Player) {
				...
				player.log("MapData",
						"Player on map: " + tChar.Name + " " + tChar.Level + "lvl, " + tChar.Authority + " authority");
				player.addGameEvent(new AdminOnMapGameEvent());
		...
		case "drop":
			...
			try {
				...
				player.log("Item Drop", "Item dropped: " + i.Pos.toString() + " Name: " + Resources.getItem(i.VNUM).Name
						+ " Owner: " + i.OwnerID);
			} catch (Exception e) {
				e.printStackTrace();
			}
			player.addGameEvent(new ItemDropGameEvent(i));
			break;
		...
		case "rest":
			...
			if (packet.getIntParameter(2) == player.id) {
				player.IsSitting = packet.getIntParameter(3) == 1;// TODO make
																	// it for
																	// all
			}
			break;
