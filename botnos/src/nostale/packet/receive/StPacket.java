...
public class StPacket extends Packet {
	/*
	 * $"st 1 {CharacterId} {Level} {HeroLevel} {(int)(Hp / (float)HPLoad() *
	 * 100)} {(int)(Mp / (float)MPLoad() * 100)} {Hp} {Mp} {Buff.Replace(s =>
	 * !s.StaticBuff).Aggregate(string.Empty, (current, buff) => current +
	 * $" {buff.Card.CardId}")}" $"st 3 {ncifPacket.TargetId}
	 * {monsterinfo.Level} {monsterinfo.HeroLevel}
	 * {(int)((float)monster.CurrentHp / (float)monster.Monster.MaxHP * 100)}
	 * {(int)((float)monster.CurrentMp / (float)monster.Monster.MaxMP * 100)}
	 * {monster.CurrentHp} {monster.CurrentMp} {monster.Buff.Replace(s =>
	 * !s.StaticBuff).Aggregate(string.Empty, (current, buff) => current +
	 * $" {buff.Card.CardId}.{buff.Level}")}"
	 */
